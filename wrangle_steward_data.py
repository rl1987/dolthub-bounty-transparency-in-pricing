...
def process_df_mid(df_mid, ccn, ein, url):
    ...
    df_mid["modifier"] = df_mid["modifier"].apply(
        lambda modifier: "na" if len(modifier.strip()) == 0 else modifier
    )
    df_mid["rev_code"] = df_mid["rev_code"].fillna("na")
    ...
    df_mid["payer_category"] = df_mid["payer_orig"].apply(
        payer_category_from_payer_orig
    )

    if not "patient_class" in df_mid.columns:
        df_mid["patient_class"] = df_mid["payer_orig"].apply(
            patient_class_from_payer_orig
        )
    else:
        df_mid["patient_class"] = (
            df_mid["patient_class"]
    ...
    if "file_last_updated" in df_mid.columns:
        df_mid["file_last_updated"] = df_mid["file_last_updated"].apply(
            lambda file_last_updated: parse_datetime(file_last_updated)
            .isoformat()
            .split("T")[0]
        )
    df_mid["plan_orig"] = "na"
    ...
    df_mid["rate"] = df_mid["rate"].apply(
        lambda rate: None if type(rate) == str and rate.startswith("ERROR") else rate
    )
...
def convert_df1(df_in, ccn, ein, url):
    ...
    df_mid = pd.melt(
        df_mid, id_vars=remaining_columns, var_name="payer_orig", value_name="rate"
    )
...
def convert_df(ccn, ein, url):
    ...
    df_in = pd.read_csv(
        filename,
        dtype={"REV CODE": str, "Rev Code": str, "DRG": str},
        encoding="latin-1",
        low_memory=False,
    )
