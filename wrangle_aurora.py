...
def convert_dataframe(df_in, ccn):
    ...
    df_mid['rev_code'] = df_mid['rev_code'].apply(lambda rev_code: rev_code.zfill(4) if type(rev_code) == str else rev_code)

    df_mid['local_code'] = None
    df_mid['local_code'] = df_mid['hcpcs_cpt'].apply(
        lambda hcpcs_cpt: hcpcs_cpt if type(hcpcs_cpt) == str and (not code_is_cpt(hcpcs_cpt) and not code_is_hcpcs(hcpcs_cpt)) else None
    )
    ...
    df_mid['payer_category'] = df_mid['payer_name'].apply(get_payer_category_from_payer_name)
