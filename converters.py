...
class AuroraXMLConverter(AbstractStandardChargesConverter):
    ...
    def convert(self, url, file_path, ccn):
        ...
        money_columns = list(filter(lambda c: c.startswith("_"), columns)) + [
            "Self_Pay",
            "Min",
            "Max",
        ]

        df_in[money_columns] = df_in[money_columns].apply(
            lambda values: cleanup_values(values)
        )
        ...
        df_intermediate = df_intermediate.rename(
            columns={
                "variable": "payer_desc",
                "value": "rate",
        ...
        df_intermediate["rev_code"] = df_intermediate["rev_code"].apply(
            lambda rev_code: rev_code.split(".")[0]
        )
        df_intermediate["rev_code"] = df_intermediate["rev_code"].apply(
            pad_rev_code_if_needed
        )
        df_intermediate["patient_class"] = (
            df_intermediate["Type"]
            .replace("CHARGE", "na")
            .replace("IP DRG*", "inpatient")
            .replace("OP PROC*", "outpatient")
        )
        df_intermediate["code_type"] = (
            df_intermediate["Type"]
            .replace("IP DRG*", "ms-drg")
            .replace("OP PROC*", "hcpcs_cpt")
            .replace("CHARGE", "cdm")
        )
        df_intermediate["code_meta"] = (
            df_intermediate["Type"]
            .replace("IP DRG*", "drg")
            .replace("OP PROC*", "cpt")
            .replace("CHARGE", "cdm")
        ...
        df_intermediate["payer_category"] = df_intermediate["payer_desc"].apply(
            get_payer_category_from_payer_desc
        )
        ...
        df_intermediate["payer_name"] = df_intermediate["payer_desc"].apply(
            get_payer_name_from_payer_desc
        )
        ...
        df_intermediate[
            "file_last_updated"
        ] = "2023-01-01"  # FIXME: refrain from hardcoding this; determine this field from _Fee column name
        ...
        df_intermediate["plan_type"] = df_intermediate["payer_desc"].apply(
            get_plan_type_from_payer_desc
        )
