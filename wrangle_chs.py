...
def payer_category_from_payer_name(payer_name):
    if payer_name == "Gross Charge":
        ...
        payer_name == "DISCOUNTED CASH PRICE INPATIENT"
        or payer_name == "DISCOUNTED CASH PRICE OUTPATIENT"
    ):
        return "cash"
...
def convert_dataframe(df_in, ccn):
    ...
    df_mid.loc[df_mid["local_code"] == "DRG", "ms_drg"] = df_mid[
        df_mid["local_code"] == "DRG"
    ]["hcpcs_cpt"].str.zfill(3)
    df_mid["ms_drg"] = df_mid["ms_drg"].apply(
        lambda ms_drg: ms_drg.replace(".00", "").zfill(3)
        if type(ms_drg) == str and ms_drg.endswith(".00")
        else ms_drg
    )

    # HACK for Ponca city hospital.
    df_mid["hcpcs_cpt"] = df_mid["hcpcs_cpt"].apply(
        lambda cpt: cpt.replace(".00", "")
        if type(cpt) == str and cpt.endswith(".00")
        else cpt
    )
    df_mid["hcpcs_cpt"] = df_mid["hcpcs_cpt"].apply(
        lambda cpt: None
        if type(cpt) == str and len(cpt) < 5 and cpt.isnumeric()
        else cpt
    )
    df_mid["hcpcs_cpt"] = df_mid["hcpcs_cpt"].apply(
        lambda cpt: cpt.replace(".00", "")
    ...
    df_mid.loc[~df_mid["local_code"].str.isnumeric(), "line_type"] = df_mid[
        ~df_mid["local_code"].str.isnumeric()
    ]["local_code"]
    df_mid["local_code"] = df_mid["local_code"].apply(
        lambda local_code: local_code.replace(".00", "")
        if type(local_code) == str and local_code.endswith(".00")
        else local_code
    )
    ...
    df_mid["plan_name"] = df_mid["payer_name"].apply(
        lambda payer_name: payer_name.split(" - ")[-1].strip()
        if " - " in payer_name
        else None
    )
