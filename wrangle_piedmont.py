...
def convert_dataframe(df_in, ccn):
    ...
    if "setting" in df_mid.columns:
        ...
        df_mid["setting"] = None
        df_mid.loc[
            df_mid["additional_generic_notes"].str.startswith("Inpatient"), "setting"
        ] = "inpatient"
        df_mid.loc[
            df_mid["additional_generic_notes"].str.startswith("Outpatient"), "setting"
        ] = "outpatient"

    df_mid.loc[df_mid["ms_drg"].notnull(), "ms_drg"] = df_mid[
        df_mid["ms_drg"].notnull()
    ]["ms_drg"].str.replace("MS", "")
    df_mid.loc[df_mid["rev_code"].notnull(), "rev_code"] = df_mid[
        df_mid["rev_code"].notnull()
    ]["rev_code"].str.zfill(4)
