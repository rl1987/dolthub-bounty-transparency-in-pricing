...
def convert_chunk(chunk, ccn):
    ...
    if "modifiers" in df_mid.columns and "Modifier2" in df_mid.columns:
        df_mid["modifiers"] = df_mid[["modifiers", "Modifier2"]].apply(
            lambda row: unify_modifiers(row["modifiers"], row["Modifier2"]), axis=1
        )
    ...
    df_mid["setting"] = None
    df_mid.loc[
        df_mid["additional_generic_notes"] == "Inpatient", "setting"
    ] = "inpatient"
    df_mid.loc[
        df_mid["additional_generic_notes"] == "Outpatient", "setting"
    ] = "outpatient"
    ...
    df_mid["hcpcs_cpt_broken"] = df_mid["hcpcs_cpt"].apply(
        lambda hcpcs_cpt: 0 if code_is_cpt(hcpcs_cpt) or code_is_hcpcs(hcpcs_cpt) else 1
    )
    df_mid.loc[df_mid["hcpcs_cpt_broken"] == 1, "code"] = df_mid[
        df_mid["hcpcs_cpt_broken"] == 1
    ]["hcpcs_cpt"]
    df_mid.loc[df_mid["hcpcs_cpt_broken"] == 1, "hcpcs_cpt"] = ""

    del df_mid["hcpcs_cpt_broken"]

    df_mid.loc[df_mid["hcpcs_cpt"].str.isalpha(), "code"] = df_mid[
        df_mid["hcpcs_cpt"].str.isalpha()
    ]["hcpcs_cpt"]
    df_mid["hcpcs_cpt"] = df_mid["hcpcs_cpt"].apply(
        lambda cpt: "" if len(cpt) != 5 else cpt.upper()
    )
    ...
    if "ms_drg" in df_mid.columns:
        ...
        df_mid.loc[df_mid["ms_drg"] != "", "ms_drg"] = df_mid[df_mid["ms_drg"] != ""][
            "ms_drg"
        ].apply(lambda drg: str(drg).replace(".0", "").zfill(3))

    if not "ndc" in df_mid.columns:
        df_mid["ndc"] = None

    # e.g. 12870-0001-1-99-100
    df_mid["ndc"] = df_mid["ndc"].apply(
        lambda ndc: ndc[:13] if type(ndc) == str and len(ndc) >= 14 else ndc
    )
...
def perform_task(h_f, ccn, app_url, transparency_page):
    ...
    js_link = tree.xpath('//script[contains(@src, "/PTT/extnet/extnet-init-js")]/@src')[
        0
    ]
    js_url = urljoin(resp.url, js_link)
    resp1 = requests.get(js_url)
    parsed = js2xml.parse(resp1.text)
    db_name = parsed.xpath(
        '//object[./property/string[text()="App.hdnDB_Container"]]/property[@name="value"]/string/text()'
    )[0]
