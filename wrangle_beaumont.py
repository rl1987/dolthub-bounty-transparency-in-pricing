...
def payer_category_from_payer(payer):
    if payer == "GROSS CHARGE" or payer == "RECONSTRUCTED CHARGE":
        ...
        payer == "DEIDENTIFIED MIN"
        or payer == "DE-IDENTIFIED  MINIMUM"
        or payer == "Min Price"
    ):
        return "min"
    elif (
        payer == "DEIDENTIFIED MAX"
        or payer == "DE-IDENTIFIED MAXIMUM"
        or payer == "Max Price"
    ):
        return "max"
...
def convert_dataframe(df_in, ccn):
    ...
    df_mid.loc[
        df_mid["standard_charge"].str.isalpha(), "additional_payer_specific_notes"
    ] = df_mid[df_mid["standard_charge"].str.isalpha()]["standard_charge"]
    ...
    if "code" in df_mid.columns:
        df_mid.loc[df_mid["line_type"] == "DRG", "ms_drg"] = df_mid[
            df_mid["line_type"] == "DRG"
        ]["code"].apply(lambda code: code.split(" ")[-1])

        df_mid["code"] = df_mid["code"].fillna("")
        df_mid["code"] = df_mid["code"].apply(
            lambda code: str(int(code))
            if type(code) == int or type(code) == float
            else code
        )

        if not "hcpcs_cpt" in df_mid.columns:
            df_mid["hcpcs_cpt"] = df_mid["code"].apply(
                lambda code: code if len(code) == 5 else None
            )
    ...
    df_mid["eapg"] = df_mid.apply(
        lambda row: None
        if type(row["eapg"]) == str and row["eapg"].startswith("Custom")
        else row["eapg"],
        axis=1,
    )

    df_mid["hcpcs_cpt"] = df_mid["hcpcs_cpt"].apply(
        lambda cpt: cpt.replace(" ", "") if type(cpt) == str else cpt
    )

    df_mid["hcpcs_cpt"] = df_mid["hcpcs_cpt"].fillna("")

    df_mid.loc[df_mid["hcpcs_cpt"].str.len() > 5, "code"] = df_mid[
        df_mid["hcpcs_cpt"].str.len() > 5
    ]["hcpcs_cpt"]
    df_mid.loc[df_mid["hcpcs_cpt"].str.len() > 5, "hcpcs_cpt"] = None
    df_mid.loc[df_mid["hcpcs_cpt"].str.len() == 4, "code"] = df_mid[
        df_mid["hcpcs_cpt"].str.len() == 4
    ]["hcpcs_cpt"]
    df_mid.loc[df_mid["hcpcs_cpt"].str.len() == 4, "hcpcs_cpt"] = None

    df_mid.loc[
        df_mid["additional_generic_notes"] == "Office Clinic Provider Fee", "line_type"
    ] = "office clinic provider fee"
    df_mid.loc[
        df_mid["additional_generic_notes"]
        == "Hospital Outpatient Department Provider Fee",
        "line_type",
    ] = "hosp. OP dept. provider fee"
