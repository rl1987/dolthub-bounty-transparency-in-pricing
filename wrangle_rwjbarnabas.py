...
from helpers import (
    cleanup_dollar_value,
    derive_ein_from_filename,
...
def convert_dataframe(df_in, ccn):
    ...
    df_mid["ms_drg"] = df_mid["code"].apply(
        lambda code: str(code) if code is not None and len(str(code)) == 3 else None
    )
    df_mid["hcpcs_cpt"] = df_mid["code"].apply(
        lambda code: str(code) if code is not None and len(str(code)) == 5 else None
    )
